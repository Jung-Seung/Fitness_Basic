...
const patch = require("./galleryUpload.js")
...
//fileupload
app.post("/person/upload", upload, (req, res) => {
...
app.post("/gallery", (req, res) => {
  ...
  const { title, contents, writer, writeDate } = req.body;
  db.query(
    `insert into gallery(title,contents,writer,writeDate)
    values('${title}','${contents}','${writer}','${writeDate}')`,
    (err, data) => {
      if (!err) {
        res.send(data);
...
app.post("/gallery/upload", patch, (req, res) => {
...
app.put("/gallery/:no", (req, res) => {
  ...
  const { title, contents } = req.body;
  db.query(
    `update gallery set title='${title}',contents='${contents}' where no=${no}`,
    (err, data) => {
      if (!err) {
        res.send(data);
      } else {
